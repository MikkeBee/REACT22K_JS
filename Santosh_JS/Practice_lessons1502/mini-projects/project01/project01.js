...
let resultTable = document.querySelector("#results-list");
let spaceShip = document.querySelector("#app-search");

const showResults = (query = "") => {
  const cleanInput = query.trim().toLowerCase();
  const filteredInput = data.filter((item) =>
    item.spacecraft.toLowerCase().includes(cleanInput)
  );
  resultTable.innerHTML = "";
  filteredInput.forEach((item) => {
    resultTable.insertAdjacentHTML("beforeend", `<li>${item.spacecraft}</li>`);
  });
};

spaceShip.addEventListener("keyup", () => {
  showResults(spaceShip.value);
});
